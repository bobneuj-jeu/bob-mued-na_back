...
exports.getFridgeIngredients = async (req, res) => {
    ...
    const query = 'SELECT * FROM fridge_ingredients WHERE userId = ?'; // 재료 조회 쿼리
    let conn;
    try {
...
exports.addFridgeIngredient = async (req, res) => {
    const { userId, name, quantity } = req.body; // 요청에서 사용자 ID, 재료명, 수량 가져오기
...
exports.updateFridgeIngredient = async (req, res) => {
    ...
    `;
    try {
        const result = await pool.query(query, [quantity, userId, name]); // 재료 수량 수정
...
exports.deleteFridgeIngredient = async (req, res) => {
    const { userId, name } = req.params; // URL 매개변수에서 사용자 ID와 재료명 가져오기

    const query = 'DELETE FROM fridge_ingredients WHERE userId = ? AND name = ?'; // 재료 삭제 쿼리
    try {
        const result = await pool.query(query, [userId, name]); // 재료 삭제
        if (result[0].affectedRows === 0) {
