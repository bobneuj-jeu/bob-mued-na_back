...
exports.addFridgeIngredient = async (req, res) => {
    ...
    try {
        await pool.query(query, [userId, ingredient_name, quantity]); // 재료 추가
...
exports.updateFridgeIngredient = async (req, res) => {
    ...
    try {
        const [result] = await pool.query(query, [quantity, userId, ingredient_name]); // 재료 수량 수정
...
exports.deleteFridgeIngredient = async (req, res) => {
    ...
    try {
        const [result] = await pool.query(query, [userId, ingredient_name]); // 재료 삭제
        if (result.affectedRows === 0) {
