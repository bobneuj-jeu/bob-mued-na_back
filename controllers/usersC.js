...
exports.signup = async (req, res) => {
    const { userId, password, allergy, diabetes, AnythingElse } = req.body; // 요청에서 사용자 정보 가져오기
    const hashedPassword = await bcrypt.hash(password, 10); // 비밀번호 해시화

    const query = 'INSERT INTO users (userId, password, allergy, diabetes, AnythingElse) VALUES (?, ?, ?, ?, ?)';
    try {
        await pool.query(query, [userId, hashedPassword, allergy, diabetes, AnythingElse]); // DB에 사용자 정보 삽입
...
exports.login = async (req, res) => {
    ...
    try {
        const [users] = await pool.query(query, [userId]); // 사용자 정보 조회
        const user = users[0]; // 조회된 사용자 정보 가져오기
